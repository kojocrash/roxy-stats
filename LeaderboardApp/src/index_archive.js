...
// TODOK: if avatar fails to load, automatically fetch & update avatar url
...
function Follower(props) {
    ...
    return (
        <tr id={"follower"+rank} className={`followerRow row${rowType} id${id}`} style={rankColor ? {color: rankColor} : null}>
            <td className="followerRank">{String(rank).padStart(2, '0')}</td>
            <td className="followerAvatar">
                <Avatar src={disableAvatars ? "placeholderAvatar.png" : avatar}>
                    <Outline inner={"var(--pfp-border-color-secondary)"} innerWidth={"var(--pfp-border-inner-width)"} outer={rankColor ? rankColor : "var(--pfp-border-color)"} outerWidth={"var(--pfp-border-outer-width)"} contain={false}/>
                </Avatar>
            </td>
            <td className="followerName">{name}</td>{/* TODOK: TRUNCATE (useEffect + ref/querySelector) */}
...
function Leaderboard(props) {
    ...
    return (
        <div id="leaderboard">
            ...
            <div id="searchSpacer"/>
            <div id="rankings" style={{borderTop: "1px solid var(--separator);"}}>
                <table id="followers">
                    <tbody>
                        <tr id="header">
